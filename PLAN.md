...
>
> **Reference build R1 shipped 2026-08-23:** `workspace/demo/` now contains
> the working mockup of capabilities 1 and 2 (bank chrome, RAU inventory +
> intake pipeline + map builder + metadata survey, applicability workbench +
> libraries + rubric) on synthetic data at true scale, generated by
> `tools-dev/generate-data.js`. Zero-console-error verified over `file://`.
>
> **Transfer gateway constraint discovered 2026-08-24 (supersedes the ZIP
> crossing above):** the corporate transfer gateway strips `.js` and `.py`,
> and it recurses INTO archives — a `.zip` of `.js` arrives with every `.js`
> removed. Only `.md`, a `.zip` whose members are themselves all allowed
> types, and non-macro Office files (`.docx`/`.xlsx`/`.pptx`) cross intact.
> This does **not** touch the core build model: the app was always *born
> inside* from the `.md` kit via Copilot, and the kit (`.md`) + runbook
> (macro-free `.docx`) still cross. What it breaks is the shortcut of shipping
> the prebuilt reference app as a `.js` ZIP. Restored by
> `tools-dev/build-crossing-kit.js`, which repackages the reference app's code
> (388 KB, 19 files, no data) as allowed-type files that reconstitute
> byte-for-byte inside — see `workspace/crossing/RECONSTITUTE.md`. Real data
> is still produced inside from CSVs and never crosses; synthetic data only
> needs to cross for an inside synthetic demo (`--with-data`).
