/* GRC modules/demo.js v1.0.0 2026-08-23 */
...
(function () {
  ...
  function scenes(ctx) {
    ...
    var s = [
      { route: "home", title: "One platform, ten capabilities", text: "This is a clickable design proposal for the future GRC. The map shows all ten capabilities and how they feed each other; this release builds capabilities 1 and 2 end to end. Everything you are about to see runs on data shaped exactly like ours: 850 RAUs, 90 risk events, 8,000 MCRs." },
      { route: "raus", title: "Capability 1 - the RAU inventory", text: "A RAU is the intersection of a business and a service, created at the SubLOB level. Filter the 850 by line of business, category, or risk-ID status. Every column here is a real attribute the platform maintains." },
      { route: "raus/" + story.id, title: "One RAU's whole story", text: "Demographics describe the unit; attributes OBLIGATE it - they drive applicability, signal matching, and scoping. Check the Metadata survey tab: every answer shows where it came from - derived from a map step, from services, or asked directly. The Process map tab shows the handoffs that wire this RAU to its counterparties." },
    ...
    if (mapping) s.push({
      ...
      text: "Each intake bullet becomes a phase; the requester expands it 2-10 steps with the assistant coaching - what comes next, where are the handoffs, what happens on failure. The standards checklist gates progression: no expert needed, and no map leaves here below standard. Try adding a step."
    });
    if (meta) s.push({
    ...
    s.push(
      {
        ...
        text: "The engine scores all 90 risk events (and the MCRs beneath the compliance ones) against this RAU's metadata using one standardized rubric. Likely candidates confirm in a click; the exclusions panel shows what the survey suppressed; the ambiguous middle is where the intelligence earns its keep."
      },
      {
      ...
      },
      {
        ...
        text: "Eight thematic categories, 1-5 anchors, fixed weights, published bands - identical for all 850 RAUs. Applicability stops being 850 opinions and becomes one defensible calculation plus documented human judgment."
      },
      {
        route: "mcrlib", title: "8,000 MCRs, published from RRCM",
        text: "Major Compliance Requirements arrive read-only from RRCM, each aligned upstream to a parent compliance risk event. About 2,000 head MCRs carry 80% of RCSA frequency - the library reflects that shape. Search anything."
      },
      {
  ...
  GRC.register({
    id: "demo", version: "1.0.0", tab: "Home",
